...
be satisfied.
'''
def MOM_dp_process(clauses, symbols, solution):
    ...
    while (MOM_find_unit_clause(clauses)!= False):
        unit_clause_symbol = MOM_find_unit_clause(clauses)
        if(MOM_check_polarity_clause(unit_clause_symbol, clauses) == False):
            key = next(iter(unit_clause_symbol.keys()))
            value = next(iter(unit_clause_symbol.values()))
            MOM_simplify_clauses(unit_clause_symbol,clauses)
    ...
    exsit = MOM_heuristic(clauses)
    settings.search_count += 1
    solution[exsit] = 1
    if (MOM_check_polarity_clause({exsit: 1}, clauses) == False):
        if (MOM_dp_process(MOM_simplify_clauses({exsit: 1}, clauses), symbols, solution)):
            return solution
        else:
            settings.backtrack_count += 1
            solution[exsit] = -1
            if (MOM_check_polarity_clause({exsit: -1}, clauses_backup) == False):
                return MOM_dp_process(MOM_simplify_clauses({exsit: -1}, clauses_backup), symbols, solution)


def MOM_find_unit_clause(clauses):
    unit_clause = False
...
def MOM_check_polarity_clause(unit_clause, clauses):
...
def MOM_simplify_clauses(symbol_to_test, clauses):
    clauses_to_delete  = []
